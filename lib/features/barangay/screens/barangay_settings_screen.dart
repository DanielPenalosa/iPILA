...
class BarangaySettingsScreen extends StatefulWidget {
  const BarangaySettingsScreen({super.key});

  @override
  State<BarangaySettingsScreen> createState() => _BarangaySettingsScreenState();
}

class _BarangaySettingsScreenState extends State<BarangaySettingsScreen> {
  int _section = 0;
          ...
          child: Column(
            ...
            children: [
              const Padding(
                ...
                child: Text('Settings',
                    style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700,
                        color: Color(0xFF111111), letterSpacing: -0.4)),
              ),
              ...
              ...List.generate(_sections.length, (i) {
                ...
                return _SectionTile(
                  label: s.label, icon: s.icon, active: active,
  ...
  Widget _buildContent() {
    switch (_section) {
      case 0: return const _ProfileSection();
      case 1: return const _ChangePasswordSection();
      case 2: return const _AboutSection();
...
class _SectionTile extends StatefulWidget {
  ...
  const _SectionTile({required this.label, required this.icon,
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: widget.active
                ...
                : _hovered ? const Color(0xFFF5F6FA) : Colors.transparent,
          ...
          child: Row(
            children: [
              Icon(widget.icon, size: 17,
                  color: widget.active ? activeColor
                      : _hovered ? AppTheme.textDark : AppTheme.textMuted),
...
class _ProfileSectionState extends State<_ProfileSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _SectionHeader(title: 'Profile', subtitle: 'Your account information for this barangay portal.'),
            ...
            Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    user?.barangay != null ? 'Brgy. ${user!.barangay}' : 'Barangay',
          ...
          child: Column(
            children: [
              ...
              _Field(label: 'Email', controller: _emailCtrl, icon: Icons.email_outlined,
                  enabled: false, hint: 'Contact admin to change email'),
              const Divider(height: 1, color: Color(0xFFF3F4F6)),
              _Field(label: 'Phone', controller: _phoneCtrl, icon: Icons.phone_outlined,
                  keyboardType: TextInputType.phone),
              const Divider(height: 1, color: Color(0xFFF3F4F6)),
              _ReadOnlyField(
                label: 'Barangay',
                value: user?.barangay != null ? 'Brgy. ${user!.barangay}' : '—',
                icon: Icons.location_city_outlined,
              ),
              const Divider(height: 1, color: Color(0xFFF3F4F6)),
              const _ReadOnlyField(label: 'Role', value: 'Barangay Staff', icon: Icons.badge_outlined),
            ],
          ),
        ),
              ...
              try {
                ...
                if (uid != null) {
                  await FirebaseFirestore.instance
                      ...
                      .doc(uid)
                      .update({'fullName': _nameCtrl.text.trim(), 'phone': _phoneCtrl.text.trim()});
                }
                if (mounted) {
    ...
    return Column(
      ...
      children: [
        _SectionHeader(title: 'Change Password',
            subtitle: 'Update your login password. Use a strong, unique password.'),
        const SizedBox(height: 24),
        _Card(
          child: Column(
            children: [
              _PasswordField(label: 'Current Password', controller: _currentCtrl,
                  obscure: _obscureCurrent,
                  onToggle: () => setState(() => _obscureCurrent = !_obscureCurrent)),
              const Divider(height: 1, color: Color(0xFFF3F4F6)),
              _PasswordField(label: 'New Password', controller: _newCtrl,
                  obscure: _obscureNew,
                  onToggle: () => setState(() => _obscureNew = !_obscureNew)),
              const Divider(height: 1, color: Color(0xFFF3F4F6)),
              _PasswordField(label: 'Confirm New Password', controller: _confirmCtrl,
...
class _AboutSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _SectionHeader(title: 'About', subtitle: 'Information about the iPILA system.'),
        const SizedBox(height: 24),
        const _Card(
          child: Column(
            children: [
              _ReadOnlyField(label: 'App Name', value: 'iPILA', icon: Icons.apps_rounded),
              Divider(height: 1, color: Color(0xFFF3F4F6)),
              _ReadOnlyField(label: 'Municipality', value: 'Municipality of Pila, Laguna',
                  icon: Icons.location_city_outlined),
              Divider(height: 1, color: Color(0xFFF3F4F6)),
              _ReadOnlyField(label: 'Portal', value: 'Barangay Staff Portal',
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 2),
                      Text('Log out of the barangay portal.',
...
class _Field extends StatelessWidget {
  ...
  const _Field({required this.label, required this.controller, required this.icon,
      this.enabled = true, this.hint, this.keyboardType});
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 130,
              child: Text(label, style: const TextStyle(fontSize: 13, color: Color(0xFF6B7280)))),
          Expanded(child: Text(value, style: const TextStyle(fontSize: 13, color: Color(0xFF111111)))),
        ],
      ),
    );
  }
...
class _SaveButton extends StatelessWidget {
  ...
  final String label;
  const _SaveButton({required this.saving, required this.onSave, this.label = 'Save Changes'});
