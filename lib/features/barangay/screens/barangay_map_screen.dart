...
class _BarangayMapScreenState extends State<BarangayMapScreen> {
  ...
  List<ReportModel> _applyFilters(List<ReportModel> all) {
    return all.where((r) {
      if (_filterStatus != 'All' && r.currentStatus != _filterStatus) return false;
      return true;
    }).toList();
  }
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: DropdownButton<String>(
                      ...
                      isDense: true,
                      icon: const Icon(Icons.map_outlined, size: 16, color: Color(0xFF9CA3AF)),
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      ('Needs Revision', AppTheme.statusColor('Needs Revision')),
                      ...
                      children: [
                        ...
                        Text(e.$1, style: const TextStyle(fontSize: 11, color: Color(0xFF9CA3AF))),
                      ],
                    )).toList(),
                  ),
                ],
                  ...
                  Expanded(
                    ...
                    child: Stack(
                      children: [
                          ...
                          children: [
                            ...
                            MarkerLayer(
                              markers: filtered.map((r) {
                                final color = AppTheme.statusColor(r.currentStatus);
                                return Marker(
                        ...
                        Positioned(
                          top: 16, left: 16,
                          child: _StatsCard(reports: filtered),
                        ),
                        Positioned(
                          right: 16, bottom: 24,
                      ...
                      children: [
                        ...
                        Expanded(
                          child: filtered.isEmpty
                              ? const Center(child: Text('No reports match filters.',
                                  ...
                                  itemBuilder: (_, i) {
                                    ...
                                    return _ListItem(
                                      ...
                                      onTap: () => _mapController.move(
                                          LatLng(r.latitude, r.longitude), 16),
                                    );
                                  },
                                ),
                        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          const Text('Overview', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
              color: Color(0xFF9CA3AF))),
          const SizedBox(height: 6),
          Text('${reports.length}', style: const TextStyle(fontSize: 26,
              fontWeight: FontWeight.w700, color: Color(0xFF111111), letterSpacing: -1)),
          const Text('reports', style: TextStyle(fontSize: 11, color: Color(0xFF9CA3AF))),
          if (byStatus.isNotEmpty) ...[
            const SizedBox(height: 10),
            const Divider(height: 1, color: Color(0xFFF3F4F6)),
            const SizedBox(height: 10),
            ...byStatus.entries.map((e) {
              final c = AppTheme.statusColor(e.key);
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Text(e.key, style: const TextStyle(fontSize: 11,
                        color: Color(0xFF6B7280)), overflow: TextOverflow.ellipsis)),
                    Text('${e.value}', style: TextStyle(fontSize: 11,
...
class _ZoomControls extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        _ZoomBtn(icon: Icons.add,
            onTap: () => controller.move(controller.camera.center, controller.camera.zoom + 1)),
        const SizedBox(height: 4),
        _ZoomBtn(icon: Icons.remove,
            onTap: () => controller.move(controller.camera.center, controller.camera.zoom - 1)),
      ],
    );
  }
...
class _ListItem extends StatefulWidget {
  ...
  final Color color;
  final VoidCallback onTap;
  const _ListItem({required this.report, required this.color, required this.onTap});
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border(left: BorderSide(
                color: _hovered ? widget.color : Colors.transparent, width: 3)),
          ),
          child: Row(
            children: [
              AnimatedContainer(
                duration: const Duration(milliseconds: 150),
                width: _hovered ? 10 : 8, height: _hovered ? 10 : 8,
                decoration: BoxDecoration(color: widget.color, shape: BoxShape.circle,
                    boxShadow: _hovered ? [BoxShadow(
                        color: widget.color.withValues(alpha: 0.4), blurRadius: 5)] : []),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(widget.report.category,
                        style: TextStyle(fontSize: 13,
                            ...
                            color: _hovered ? widget.color : const Color(0xFF111111))),
                    const SizedBox(height: 2),
                    Text('Brgy. ${widget.report.barangay}',
              ...
              Container(
                ...
                child: Text(widget.report.currentStatus,
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [statusColor.withValues(alpha: 0.15),
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.location_on_outlined, color: statusColor, size: 22),
                    ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(
                          children: [
                            _DialogRow(icon: Icons.person_outline, label: 'Reporter',
                                value: report.isAnonymous ? 'Anonymous' : report.userFullName),
                            const Divider(height: 16, color: Color(0xFFE5E7EB)),
                            _DialogRow(icon: Icons.location_on_outlined, label: 'Address',
                                value: report.address.isNotEmpty ? report.address : 'Brgy. ${report.barangay}'),
                            const Divider(height: 16, color: Color(0xFFE5E7EB)),
                            _DialogRow(icon: Icons.calendar_today_outlined, label: 'Reported',
                      ...
                      const Text('DESCRIPTION', style: TextStyle(fontSize: 10,
                          fontWeight: FontWeight.w700, color: Color(0xFF9CA3AF), letterSpacing: 0.8)),
                      const SizedBox(height: 6),
                      Container(
                        ...
                        child: Text(report.description.isNotEmpty ? report.description : '—',
                            style: const TextStyle(fontSize: 13, color: Color(0xFF374151), height: 1.5)),
                      ),
                      if (report.photoUrls.isNotEmpty) ...[
                        const SizedBox(height: 16),
                        const Text('PHOTOS', style: TextStyle(fontSize: 10,
                            fontWeight: FontWeight.w700, color: Color(0xFF9CA3AF), letterSpacing: 0.8)),
                  ...
                  children: [
                    ...
                    FilledButton.icon(
                      onPressed: () {
                        ...
                        context.push('/barangay/reports/${report.id}');
                      },
...
class _DialogRow extends StatelessWidget {
  ...
  final String label;
  final String value;
  const _DialogRow({required this.icon, required this.label, required this.value});
